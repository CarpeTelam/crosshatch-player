...
1. **A script runtime.** It is a sandboxed interpreter, probably Lua 5.4 compiled as C, following SUMI's MIT-licensed recipe `[ASSUMPTION: engine settled by a footprint spike against Berry]`. Its API is designed for touch and e-ink from the start: touch events, greyscale drawing, and control over which parts of the screen refresh and when. A script error shows an error screen instead of crashing the device.
...
3. **Install by upload.** A game is a single package: a manifest, the script, and an icon `[ASSUMPTION: exact format decided in architecture]`. You upload it through the web file manager, or a Games page modeled on the existing `.cpfont` Fonts page. The firmware validates it and lists it in a games launcher `[ASSUMPTION: installed without a reboot]`.
...
- **Fork discipline.** The runtime stays additive behind device and capability guards, so the fork keeps merging cleanly with upstream. That is the exact failure that started this project.
...
- **Later: other developers with AI.** They reach it through a game starter repo they can point an AI at. This is out of v1 scope but shapes the v1 API: it must be documented well enough for an AI to write against it.

## Success Criteria

- **Idea to device without firmware.** A new game goes from idea to playing on a device with no firmware build `[ASSUMPTION]`.
...
- **Clean upstream merges.** Merging upstream `develop` touches no game code, and the runtime's changes to upstream files stay small `[ASSUMPTION: a count set during architecture]`.
- **The restaurant test.** A child can launch a game and finish a round without adult help.
- **The API proves itself.** At least three first-party games ship as scripts, covering a solo puzzle, an open-information two-player game, and a hidden-information two-player game `[ASSUMPTION: three games]`.
...
- A computer opponent provided by the runtime `[ASSUMPTION: a script may implement its own]`.
...
- **Radio.** Is ESP-NOW reliable between two of these devices, and what does a match cost in battery? CrossPlay never measured it. We also need to confirm that the pinned Arduino-ESP32 version supports ESP-NOW v2's larger payloads.
- **State size.** Does a typical game's state fit in one radio payload? It is 250 B in ESP-NOW v1 and 1,470 B in v2.
